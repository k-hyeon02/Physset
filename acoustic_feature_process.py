...
class Acoustic_feature_process(torch.nn.Module):
    ...
    def extract_acoustic_features(self, x_real, x_imag, coherence_frames=9):
        """
        ...
        Returns:
            tuple[Tensor, list[tuple[int, int]]]
                - acoustic_features(a_ij): 모든 마이크 쌍의 5채널 acoustic feature, (B, P, 5, F, T)
        ...
        return acoustic_features, mic_pairs

    def forward(self, inputs, coherence_frames=9):
        """
        waveform input을 STFT 변환 후, acoustic feature 계산
        Args:
            inputs: (B, C, L) 형태의 다채널 waveform

        Returns:
            acoustic_features: (B, P, 5, F, T)
            mic_pairs: 마이크 pair 인덱스 목록
        """
        x_real, x_imag = self.STFT(inputs, cplx=True)
        return self.extract_acoustic_features(x_real, x_imag, coherence_frames=coherence_frames)
