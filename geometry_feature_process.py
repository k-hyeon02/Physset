...
def pair_geometry_features(
    ...
    Args:
        ...
        mic_pairs (list[tuple[int, int]]): acoustic_features와 동일한 순서의 (i, j) 인덱스 목록
        speed_of_sound (float): 음속 c [m/s]
        eps (float): division-by-zero 방지용 작은 값

    Returns:
        geometry_features (torch.Tensor): (..., P, 8) = [d_ij(3), bar_d_ij(3), l_ij(1), tau_ij_max(1)]
    ...
    geometry_features = torch.cat([d_ij, d_ij_unit, l_ij, tau_ij_max], dim=-1)  # (..., P, 8)
    ...
    return geometry_features, array_aperture